...
export function useIsAdmin() {
  ...
  const isAdminOnRegistryQuery = useQuery({
    ...
    queryFn: () =>
      client!.readContract({
        ...
        args: [adminRoleQuery.data!, address!],
      }) as Promise<boolean>,
  ...
  const isOperatorOnRegistryQuery = useQuery({
    ...
    queryFn: () =>
      client!.readContract({
        ...
        abi: SpRegistryAbi,
        functionName: 'hasRole',
        args: [operatorRoleQuery.data!, address!],
      }) as Promise<boolean>,
  ...
  return {
    ...
    isAnyAdmin:
      (isAdminOnRegistryQuery.data ||
        isOperatorOnRegistryQuery.data ||
        isAdminOnMarketQuery.data) ??
      false,
    adminRole: adminRoleQuery.data,
    operatorRole: operatorRoleQuery.data,
    isLoading:
      adminRoleQuery.isLoading ||
      operatorRoleQuery.isLoading ||
      isAdminOnRegistryQuery.isLoading ||
      isAdminOnMarketQuery.isLoading,
  }
}
